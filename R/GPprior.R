...
# squared exponnential covariance kernel
squared_exponential_covfunction <- function(b = NULL, X, sigma_hat, l) {
...
rational_quad_kernel <- function(b = NULL, X, alpha, l, sigma_hat) {
  n <- length(X)
  if (is.null(b) == TRUE){
    for (i in 1:n) {
      for (j in 1:n) {
...
gp_posterior <- function(X, Y, b, sigma_hat, choice = 1, l = NULL, alpha = NULL) {
  ...
  if (choice == 1) {
    ...
    Kb_xc <- squared_exponential_covfunction(b, Xc, sigma_hat, l)
    Kb_xt <- squared_exponential_covfunction(b, Xt, sigma_hat, l)
  }
  if (choice == 2) {
    Kc <- rational_quad_kernel(Xc, alpha, l, sigma_hat)
    Kt <- rational_quad_kernel(Xt, alpha, l, sigma_hat)
    Kb_xc <- rational_quad_kernel(b, Xc, sigma_hat, l)
    Kb_xt <- rational_quad_kernel(b, Xt, sigma_hat, l)
  }
